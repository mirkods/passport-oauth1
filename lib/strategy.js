...
/**
 *  Initialize a queue to register local settings during oauth authentication to avoid the session support.
 */
var _sessionCache = {};
...
OAuthStrategy.prototype.authenticate = function(req, options) {
  ...
  // Copy the content of the session header in the session of the request
  req.session = JSON.parse(req.headers['x-stamplay-session-v0']);
  ...
  if (req.query && req.query['oauth_token']) {
    ...
    /* restoring old session status */
    req.session = _sessionCache[req.session.sessionID];
    ...
    this._oauth.getOAuthAccessToken(oauthToken, oauthTokenSecret, oauthVerifier, function(err, token, tokenSecret, params) {
      ...
      self._loadUserProfile(token, tokenSecret, params, function(err, profile) {
        ...
        // When the oauth process is finished, its session is removed from the queue
        delete _sessionCache[req.session.sessionID];
      });
    });
  } else {
    ...
    this._oauth.getOAuthRequestToken(params, function(err, token, tokenSecret, params) {
      ...
      // Insert the actual session status in the queue to manage the oauth redirect
      _sessionCache[req.session.sessionID] = req.session;
      ...
      req.headers['x-stamplay-session'] = JSON.stringify(req.session);
